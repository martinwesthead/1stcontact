...
updated_at: '2026-06-30T01:02:05.433710+00:00'
completed_at: null
...
1. [[REQ-23]] — **D1 schema for chat sessions + reference docs**: migrations for `chat_sessions`, `chat_messages` + FTS5, `reference_docs` + FTS5. Depends on [[REQ-10]].
2. [[REQ-24]] — **Chat session API + AI memory tools**: control-app endpoints + Anthropic call refactor (tail-prime + 4 new tools).
3. [[REQ-25]] — **Builder UI chat persistence**: replace in-memory `BuilderStore.chatHistory`; session list + new-chat + infinite scroll.
4. [[REQ-26]] — **Reference docs library + initial seed**: seed content distilled from foundational DOCs / CHATs; build-time hydration of `reference_docs`.
