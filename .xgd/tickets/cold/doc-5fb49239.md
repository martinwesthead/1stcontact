...
created_by: xgd
created_at: '2026-06-30T01:01:53.811906+00:00'
updated_at: '2026-06-30T20:22:12.991517+00:00'
completed_at: null
...
**There is one kind of site.** The 1st Contact marketing site is an ordinary site built on this same model — authored, stored, and served exactly like a customer site, with no special path. The control app (builder/portal) is a separate application, not a site, and does not pass through the framework renderer.

**Filespace is a seed representation, not a permanent store.** During development, sites may be authored as files under `sites/<name>/` and then migrated into D1 — the canonical home for every live site (definition in D1, asset bytes in R2). File-backed and D1-backed are two storage representations of the same site, not two kinds of site.

**Everything is versioned, forward-only.** A revision is an immutable snapshot of the entire site — definition, assets, and metadata. The live site is always the latest revision (no separate published pointer). "Rollback" = restore an old revision into the draft and publish it as a new revision. The full storage/versioning/rendering model — file-backed now, D1 + R2 eventually — is specified in [[DOC-12]].
...
2. Attach account/site IDs to all customer records.
